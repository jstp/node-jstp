...
JSTPEndpoint.prototype.matchResourcePattern = function (dispatch) {
  var dispatchResourcePattern  = dispatch.getAsEndpoint().getResourcePattern();
  var matches     = true;
  var index       = 0;

  while (matches && index < dispatchResourcePattern.length) {
    matches = this.resourcePattern[index] == dispatchResourcePattern[index] || 
              this.resourcePattern[index] == "*" ||
              this.resourcePattern[index] == "...";              
    index ++;
  }
