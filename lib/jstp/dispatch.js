 ...
 * ---------------
 *
 * Returns a JSTPEndpoint representing the endpoint part of the Dispatch.
 * 
 * If the JSTPDispatch if of Subscription Morphology, it will map:
 *
 * - Dispatch Endpoint Resource Pattern > Resource Pattern
 * - Dispatch Method Header > Method Pattern
 * - Dispatch To Header > To Pattern
 * - Dispatch From Header > From Pattern
 *
 * If the JSTPDispatch if of other Morphology, it will map:
 *
 * - Dispatch Resource Header > Resource Pattern
 * - Dispatch Method Header > Method Pattern
 * - Dispatch To Header > To Pattern
 * - Dispatch From Header > From Pattern
 *
 * Calls `validate()` on self upon startup and propagates any generated error.
 * 
 * #### Returns
 *
 * - JSTPEndpoint endpoint
 *
 */
JSTPDispatch.prototype.getAsEndpoint = function () {
  this.validate();

  if (this.isOfSubscriptionMorphology()) 
    return new JSTPEndpoint()
               .setMethodPattern(this.getMethod())
               .setResourcePattern(this.getEndpoint().getResourcePattern())
               .setFromPattern(this.getFrom())
               .setToPattern(this.getTo());
  else
    return new JSTPEndpoint()
               .setMethodPattern(this.getMethod())
               .setResourcePattern(this.getResource())
               .setFromPattern(this.getFrom())
               .setToPattern(this.getTo());
...
JSTPDispatch.prototype.validate = function () {
  ...
  if (this.isOfAnswerMorphology()) {
    ...
    if (parseInt(this.getResource()[0]) != 101 && this.getResource().length < 3)
      throw new exceptions.
        JSTPInvalidResourceHeaderForAnswer(
          "Must have Status Code, Transaction ID and Triggering ID");

    if (
        parseInt(this.getResource()[0]) != 101 && 
        typeof this.getResource()[2] != "string" && typeof this.getResource()[2] != "number"
        )
      throw new exceptions.
        JSTPInvalidResourceHeaderForAnswer(
