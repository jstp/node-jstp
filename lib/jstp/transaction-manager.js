...
JSTPTransactionManager.prototype.stop = function (transactionID) {
  ...
  var noMoreAnswers = new JSTPDispatch();
  noMoreAnswers.setMethod("ANSWER");
  noMoreAnswers.setResource([101, transactionID]);
  noMoreAnswers.validate();
  this.engine.dispatch(noMoreAnswers);

  var releaseAnswer = new JSTPDispatch();
  releaseAnswer.setMethod("RELEASE");
  releaseAnswer.setEndpoint(
    new JSTPEndpoint()
      .setMethodPattern("ANSWER")
      .setResourcePattern(["*", "here", "*"])
    );
  this.engine.dispatch(releaseAnswer);
}
