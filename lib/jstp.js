...
var JSTP = {
  ...
  answer: function (dispatch) {
    if (isNaN(this.readyState)) {
      ...
      else console.log(color.xterm(203)("TCP client disconnected, dispatch not sent"));
    }
    else {
      ...
      else console.log(color.xterm(203)("Websocket client disconnected, dispatch not sent"));
    }
  },
  ...
  forward: function (pack) {
    ...
    console.log(color.xterm(203)(stringPack));

    clientPool[type](address, stringPack);
  },

  listen: function (options) {
    this.httpServer(options.websocket);
    this.tcpServer(options.tcp);
  },
